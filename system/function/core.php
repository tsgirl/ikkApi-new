...
function decypt_request(){
	global $client;
	$sid = intval($_GET['sid']);
	if(!$sid) throw new Exception('Illegal site.');
	$client = DB::fetch_first("SELECT * FROM client WHERE id='{$sid}'");
	if(!$client) throw new Exception('Illegal site.');
	if($client['key'] == 'BLOCKED') return array();
	$parm_string = trim(pack('H*', $_POST['parm']));
	$parm_string = authcode($parm_string, 'DECODE', $client['key']);
	if(!$parm_string) throw new Exception('Illegal key.');
	return unserialize($parm_string);
}
function encypt_result($array){
...
function get_real_domain($url){
	...
	$host = preg_replace('/[0-9]+\.(\w+\.duapp\.com)/i', '\1', $host);
	$host = preg_replace('/www\.(\w+)/i', '\1', $host);
	return $host;
}
function get_host_hash($url){
	$host = parse_url($url, PHP_URL_HOST);
...
function check_site($url){
	$sitehash = md5(get_real_domain($url));
	$client = DB::fetch_first("SELECT * FROM client_api WHERE url='{$sitehash}'");
	if($client['status']<0) showmessage('该网站在黑名单中，禁止使用此功能');
	$check = array(
		'https://me.alipay.com/kookxiang',
		'KK\'s Laboratory',
		'www.kookxiang.com',
		'www.ikk.me',
		'商业用途',
	...
	if($client['lastcheck'] > TIMESTAMP - 3600) return;
	$page = fetch($url.'member.php?action=login');
	if(!strexists($page, 'form')) return;
	foreach($check as $str){
		if(!strexists($page, $str)) add_to_blacklist($url);
	}
	$strip_page = strip_tags($page);
	if(!strexists($strip_page, 'Designed by kookxiang.')) add_to_blacklist($url);
	$style = fetch($refer.'style/custom.css');
	if(strexists($style, '.copyright')) add_to_blacklist_2($url);
	$timestamp = TIMESTAMP;
	DB::query("UPDATE client_api SET lastcheck='{$timestamp}' WHERE url='{$sitehash}'");
}
function add_to_blacklist($url){
	DB::insert('client_api', array(
		'url' => md5(get_real_domain($url)),
		'link' => $url,
		'status' => -1,
		'lastcheck' => TIMESTAMP,
		), false, true);
	if($status) showmessage('该网站在黑名单中，禁止使用此功能');
}
function add_to_blacklist_2($url){
	DB::insert('client_api', array(
		'url' => md5(get_real_domain($url)),
		'link' => $url,
		'status' => -2,
...
function getBlackList(){
	$content = trim(file_get_contents(ROOT.'./blacklist.txt'));
	$content = str_replace(array("\r\n", "\n", "\r"), array(PHP_EOL, PHP_EOL, PHP_EOL), $content);
	return explode(PHP_EOL, $content);
}
function inBlackList($url){
	$black_list = getBlackList();
	foreach($black_list as $rule){
		if(strexists($url, $rule)) return true;
	}
	return false;
}